...
use backlight::{
    AUTO_OFF_TOGGLE_USER_ID, BACKLIGHT_BRIGHTNESS_UP_USER_ID, BACKLIGHT_LAYER_COLOR_TOGGLE_USER_ID,
    BACKLIGHT_TOGGLE_USER_ID, HUE_UP_USER_ID, SATURATION_UP_USER_ID,
    UNDERGLOW_BRIGHTNESS_UP_USER_ID, UNDERGLOW_LAYER_COLOR_TOGGLE_USER_ID,
    UNDERGLOW_TOGGLE_USER_ID, backlight_vbus_task,
};
...
async fn main(spawner: Spawner) {
    info!("On the 15 v4 RMK firmware");
    ...
    // Digital read of the 5V-rail divider with an internal pull-down, the
    // same detection proven by battery-usb-backlight-check and
    // backlight-vbus-check.
    let vbus_sense = Input::new(p.P0_03, Pull::Down);

    // Keep the data line actively low until storage, USB, and BLE setup has
    // completed. The backlight task starts alongside run_all! below so its
    // power-on animation cannot delay USB enumeration.
    ...
    // 8192 bytes: the full SDC feature set (DLE + PHY update + 2M PHY + 251-byte
    // L2CAP buffers) does not fit in 4096 bytes; build_sdc then returns Err and
    // the unwrap! below panics before USB starts (confirmed on hardware with
    // the mem4096-check binary).
    ...
    // Use the MPSL timeslot-aware flash driver instead of raw NVMC so that
    // flash writes (e.g. bond storage) cannot collide with radio activity.
    // This matches the official RMK nrf52840_ble example.
    ...
    let hue_step = storage.user_setting(HUE_UP_USER_ID).await.unwrap_or(12);
    let backlight_enabled = storage
        .user_setting(BACKLIGHT_TOGGLE_USER_ID)
        .await
        .unwrap_or(1)
        != 0;
    let underglow_enabled = storage
        .user_setting(UNDERGLOW_TOGGLE_USER_ID)
        .await
        .unwrap_or(0)
        != 0;
    let saturation_step = storage
        .user_setting(SATURATION_UP_USER_ID)
        .await
        .unwrap_or(10);
    let brightness_step = storage
        .user_setting(BACKLIGHT_BRIGHTNESS_UP_USER_ID)
        .await
        .unwrap_or(1);
    let underglow_brightness_step = storage
        .user_setting(UNDERGLOW_BRIGHTNESS_UP_USER_ID)
        .await
        .unwrap_or(1);
    ...
    let (underglow_brightness_offset, brightness_migration_pending) =
        backlight::brightness_storage_state(
            brightness_step,
            underglow_brightness_step,
            brightness_model_version,
        );
    let backlight_layer_color_enabled = storage
        .user_setting(BACKLIGHT_LAYER_COLOR_TOGGLE_USER_ID)
        .await
        .unwrap_or(0)
        != 0;
    let underglow_layer_color_enabled = storage
        .user_setting(UNDERGLOW_LAYER_COLOR_TOGGLE_USER_ID)
        .await
        .unwrap_or(0)
        != 0;
    let auto_off_enabled = storage
        .user_setting(AUTO_OFF_TOGGLE_USER_ID)
        .await
        .unwrap_or(0)
        != 0;
    backlight::configure(
        ...
        backlight_layer_color_enabled,
        underglow_layer_color_enabled,
        auto_off_enabled,
        brightness_migration_pending,
    );
    let mut status_indicator =
        StatusIndicator::standalone(led_indicator_green, led_indicator_red, indicator_enabled);
    ...
    // Backlight: P1.15 is owned by PWM0 inside the task. Starting with
    // `initially_powered = false` makes the task debounce the current VBUS
    // level and play the animation concurrently with run_all! when 5V is
    // already present.
    drop(backlight_hold);
