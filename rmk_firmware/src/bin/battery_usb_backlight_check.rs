...
async fn main(_spawner: Spawner) {
    info!("On the 15 v4 battery to USB backlight check firmware");
    ...
    let mut indicator_1 = Output::new(p.P0_28, Level::High, OutputDrive::Standard);
    ...
    loop {
        if vbus_sense.is_high() {
            indicator_1.set_high();
            indicator_2.set_high();
            write_solid_color(&mut backlight, COLORS[index]);
            index = (index + 1) % COLORS.len();
            Timer::after(Duration::from_millis(500)).await;
        } else {
            indicator_1.set_high();
            indicator_2.set_low();
            write_solid_color(&mut backlight, OFF);
