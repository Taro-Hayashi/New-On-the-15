//! x15 one-piece board firmware: 3 x 32 matrix over four daisy-chained
//! 74HC595s, 66 SK6812 (6 underglow + 60 backlight), USB + BLE + Vial.
//! Same XIAO pin assignment as x7/x8; only the matrix width, LED count,
//! keymap, and Vial definition differ from the single-board x7/x8 firmware.

#![no_std]
#![no_main]

/// SK6812 chain length on x15 (see `backlight.rs`).
...
mod xiao_qspi;
mod vial_x15 {
    include!(concat!(env!("OUT_DIR"), "/config_x15_generated.rs"));
}
...
#[cfg(feature = "battery-settings-test")]
use rmk::user;
...
use vial_x15::{VIAL_KEYBOARD_DEF, VIAL_KEYBOARD_ID};
...
async fn main(spawner: Spawner) {
    info!("On the 15 v4 x15 firmware");
    ...
    // 8192 bytes: known-good SDC memory size from the x7/x8 firmware.
    ...
    let mut battery_monitor = BatteryMonitor::new(battery_adc, 2);
    let keyboard_device_config = DeviceConfig {
        vid: 0x4F54,
        pid: 0x1537,
        manufacturer: "Hayashi",
        product_name: "On the 15 v4 X15",
    ...
    let mut default_keymap = keymap_x15::get_default_keymap();
    #[cfg(feature = "battery-settings-test")]
    {
        // Dedicated test image only: Maintenance top row after BT4.
        default_keymap[1][0][6] = user!(26);
        default_keymap[1][0][16] = user!(27);
    }
